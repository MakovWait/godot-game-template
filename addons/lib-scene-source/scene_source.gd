...
func type() -> StringName:
	var state = _SceneStateSmart.from_packed_scene(_origin_scene)
	return state.nodes().root().type()
...
func _parse_origin_scene() -> _OverriddenProperties:
	...
	var origin_state = Opt.new(_origin_scene).map(_SceneStateSmart.from_packed_scene)
			...
			return scene_root_script.props().all().map(func(script_prop: _ScriptPropertySmart):
				var overriden_prop = _OverriddenProperty.new(
					...
					scene_root.props().named(script_prop.name())\
						...
						.instead_else(Opt.lazy(script_prop.default_value))
						.unwrap_or(null)
				)
...
class _OverriddenProperties:
	...
	func named(name: StringName) -> Opt:
		for prop in _props:
			if prop.name() == name:
				return Opt.new(prop)
		return Opt.new()
...
class _SceneNodeProperties:
	...
	func named(prop_name: StringName) -> Opt:
		for prop in all():
